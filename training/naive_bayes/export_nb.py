...
OUT = ROOT / "models" / "naive_bayes" / "breast_cancer"
...
model = {
    ...
    "class_prior": clf.class_prior_.astype(float).tolist(),
    "theta": clf.theta_.astype(float).tolist(),
    "var": clf.var_.astype(float).tolist(),
    ...
    "profile_hint": {
        "compute_class": "lightweight_probabilistic",
        "memory_class": "low",
        "complexity": "O(classes*d)",
        "features": int(X.shape[1]),
        "classes": int(len(clf.classes_)),
