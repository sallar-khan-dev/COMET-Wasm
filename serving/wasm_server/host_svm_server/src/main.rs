...
async fn main() -> Result<()> {
    ...
    println!("Starting COMET-Wasm SVM-RBF Wasmtime Server");

    let engine = Engine::default();

    let module_path =
        "../../../wasm/tenant_svm_real/target/wasm32-unknown-unknown/release/tenant_svm_real.wasm";
    ...
    println!("SVM Wasm module loaded");
    println!("Feature count: {}", feature_count);
    println!("Support vectors: {}", sv_count);
    ...
    let addr =
        SocketAddr::from(([0, 0, 0, 0], 8060));

    println!(
        "Wasm SVM server listening on http://{}",
        addr
    );
