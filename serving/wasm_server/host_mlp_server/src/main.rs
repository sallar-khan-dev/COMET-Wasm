...
async fn main() -> Result<()> {
    ...
    println!("Starting COMET-Wasm MLP Wasmtime Server");

    let engine = Engine::default();

    let module_path =
        "../../../wasm/tenant_mlp_real/target/wasm32-unknown-unknown/release/tenant_mlp_real.wasm";
    ...
    let hidden_count_fn =
        instance.get_typed_func::<(), i32>(&mut store, "hidden_count")?;
    ...
    let hidden_count =
        hidden_count_fn.call(&mut store, ())?;

    if feature_count != N_FEATURES as i32 {
        bail!("Feature count mismatch");
    }

    let input_ptr =
        input_ptr_fn.call(&mut store, ())? as usize;

    println!("MLP Wasm module loaded");
    println!("Feature count: {}", feature_count);
    println!("Hidden units: {}", hidden_count);
    ...
    println!(
        "Wasm MLP server listening on http://{}",
        addr
    );
