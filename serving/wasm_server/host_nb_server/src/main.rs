...
async fn infer(
    ...
    let prediction = predict.call(&mut runtime.store, ()).map_err(|e| {
        (
            StatusCode::INTERNAL_SERVER_ERROR,
...
async fn main() -> Result<()> {
    println!("Starting COMET-Wasm Gaussian NB Wasmtime Server");
    ...
    println!("Wasm NB server listening on http://{}", addr);
