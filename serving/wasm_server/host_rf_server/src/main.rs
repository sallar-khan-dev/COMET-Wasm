...
async fn main() -> Result<()> {
    println!("Starting COMET-Wasm Random Forest Wasmtime Server");

    let engine = Engine::default();

    let module_path =
        "../../../wasm/tenant_rf_real/target/wasm32-unknown-unknown/release/tenant_rf_real.wasm";
    ...
    println!("Wasm RF server listening on http://{}", addr);
